class DestroyInformePorTiposDeCliente < ActiveRecord::Migration[5.2]
  def self.up
  ...
  def self.down
    self.connection.execute %Q( CREATE OR REPLACE VIEW informe_por_tipos_de_cliente_view AS
      SELECT
        tc.id, tc.descripcion, extract(year from rec.fecha_pago) anio_pago,
      ...
      GROUP BY tc.id, tc.descripcion, extract(year from rec.fecha_pago);
    )
  end
end
