class ViewClients < ActiveRecord::Migration[5.2]
  def up
    self.connection.execute %Q( CREATE OR REPLACE VIEW view_client AS
      SELECT
        tcl.id,
        tcl.codigo,
        tcl.created_at,
        ...
        (CASE WHEN tem.id IS NOT NULL THEN tem.email WHEN tpe.id IS NOT NULL THEN tpe.email ELSE tot.email END) as email,
        (CASE WHEN tcl.prospecto_at IS NULL THEN 'Si' ELSE 'No'END) as es_prospecto,
        tes.descripcion as estatus,
        ttp.descripcion as tipo_persona 
      FROM t_clientes tcl
      LEFT JOIN t_empresas tem ON tcl.persona_type = 'TEmpresa' AND tcl.persona_id = tem.id 
